  ...
  >
    ...
    <div class="flex items-center justify-between">
      <h2 class="text-2xl w-10/12 font-semibold text-heading m-0 hover:text-secondary transition-colors duration-300 ease-in-out">
        {{ project.title }}</h2>
      <div class="bg-pb rounded-2xl p-[15px] inline-block text-center">
        <img src="/icons/up-arrow-icon.svg" class="w-[15px] block" alt="Upw Arrow Icon"/>
      </div>
    </div>
    <div class="text-normal font-light text-gray-300 m-0 leading-0 hover:text-primary transition-colors duration-300 ease-in-out"
         v-html="project.content ? project.content.slice(0, 100) + '...' : ''"></div>
    <ProjectCategory
